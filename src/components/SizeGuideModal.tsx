...
export const SizeGuideModal: React.FC<SizeGuideModalProps> = ({ isOpen, onClose, settings }) => {
  if (!isOpen) return null;

  const title = settings?.infoGuideTitle || 'Guía Oficial de Tallas';
  const subtitle = settings?.infoGuideSubtitle || 'Medidas en centímetros (Aproximadas estándar)';
  const showSizes = settings?.infoGuideShowSizes ?? true;
  const items = settings?.infoGuideItems || [];
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-3 sm:p-4">
      ...
      <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-xl w-full p-5 sm:p-6 shadow-2xl z-10 text-slate-900 dark:text-slate-100 animate-scale-up space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        {/* Comparison Table (solo si está activada) */}
        {showSizes && (
        ...
        {/* Contenido configurable (tips, políticas, pasos, etc.) */}
        {items.length > 0 && (
